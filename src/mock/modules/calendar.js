...
// 宜事项
const suitItems = [
  '祭祀', '祈福', '求嗣', '开光', '塑绘', '斋醮', '订盟', '纳采', '嫁娶', '起基',
  '动土', '竖柱', '上梁', '造屋', '安门', '收财', '纳畜', '造畜椆栖', '安床',
  '修造', '动土', '置产', '破土', '启钻', '修坟', '安葬', '谢土', '赴任', '出行',
  '移徙', '祈福', '求医', '治病', '会友', '执友', '造船', '乘船', '捕捉', '进人口',
  '裁衣', '冠笄', '理发', '造桥', '作灶', '开池', '栽种', '牧养', '修饰垣墙',
  '造仓', '鼓铸', '经络', '酝酿', '开市', '立券', '交易', '纳财', '开仓',
  '出货财', '启钻', '安葬', '入殓', '移柩', '除服', '成服', '余事勿取',
  '结婚', '搬家', '装修', '开业', '入宅', '打扫', '房屋清洁', '美甲浴足'
]

// 忌事项
const avoidItems = [
  '栽种', '开渠', '放水', '针灸', '安床', '安葬', '开池', '放水', '针灸',
  '开市', '立券', '挂匾', '交易', '纳财', '造船', '入宅', '移徙', '分居',
  '安香', '出火', '出行', '祭祀', '祈福', '斋醮', '订盟', '纳采', '嫁娶',
  '起基', '动土', '竖柱', '上梁', '造屋', '合帐', '造床', '开仓', '作灶',
  '造船', '经络', '纳畜', '捕捉', '取鱼', '畋猎', '造畜椆栖', '造桥',
  '作灶', '造仓', '修造', '动土', '置产', '破土', '启钻', '修坟', '谢土',
  '安葬', '伐木', '作梁', '行丧', '理发', '剃头', '整手足甲'
]
...
const generateCalendarData = (dateStr) => {
  ...
  const lunarDayStr = lunarDay < 10 ? `初${['', '一', '二', '三', '四', '五', '六', '七', '八', '九'][lunarDay]}` :
                     lunarDay < 20 ? `十${['', '一', '二', '三', '四', '五', '六', '七', '八', '九'][lunarDay - 10]}` :
                     lunarDay < 30 ? `廿${['', '一', '二', '三', '四', '五', '六', '七', '八', '九'][lunarDay - 20]}` :
                     '三十'
